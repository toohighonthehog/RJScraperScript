...
#   stage 2 - fix all the filenames
    ...
    for full_filename in scanned_directory:
        ...
        fixed_filename = rjgen.search_for_title(full_filename)

        if len(fixed_filename) == 1:
            new_filename = fixed_filename[0] + file_extension
            if (full_filename != new_filename):
                my_logger.info(rjlog.logt(f"MOV - {full_filename} becomes {new_filename}."))
            ...
            my_logger.warning(rjlog.logt(f"MOV - {full_filename} (skipping. {str(len(fixed_filename))} results found)."))
